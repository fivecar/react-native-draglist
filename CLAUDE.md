  ...
  **The loop, not `onScroll`, is the authority on where the list is mid-drag.** This is the one invariant to keep in mind when touching any of this. `onScroll` reports lag a frame or more *and arrive unevenly*, so the loop integrates its own offset (`autoScrollOffsetRef` in flow space, `autoScrollScrollPosRef` its cartesian twin) and `updateRendering` draws against that via `effectiveScrollPos()`. Drawing against `scrollPos` instead jitters the dragged item against smoothly moving content, and leaves the loop's last command before an end-of-list pin undrawn so a release reorders to a stale slot.
  ...
  2. **Our clamp matches the platform's** — `contentExtentRef - flatWrapLayout.extent + autoScrollTrailingInsetRef`, floored at `autoScrollMinOffsetRef`, stopping when pinned so the offset can't run away past an end.
     ...
     **When in doubt, err tight.** This clamp is a *prediction* of the platform's own bound (see `RCTScrollView.m`'s `maxRect`), assembled from content size, viewport and insets, and each input is another chance to be wrong — so it matters that the two error directions are wildly asymmetric. Too tight pins early: the list stops short of content it could have reached, which is cosmetic and self-healing. Too loose breaks the invariant outright: the platform silently caps a command the loop believes landed, and the loop's offset stays ahead of reality for the rest of the drag, drawing the dragged item at an offset and dropping it in the wrong slot. Prefer a guaranteed-tight approximation to a clever exact one.
     ...
     Known gaps, all in the tight direction: the trailing inset is only learned from scroll events, so a list already at its inset-free end when the drag starts pins one round early and continues on the next move event; the near bound doesn't grow into a leading inset the list wasn't already sitting in; `contentExtentRef` is only as fresh as the last `onScroll`/`onContentSizeChange`, which a list without `getItemLayout` revises as rows mount; and `flatWrapLayout.extent` is the wrapper's extent rather than the scroll view's `layoutMeasurement`, which differ if a host's `containerStyle` pads the wrapper.
